...
my $query = CGI->new;
$query->charset('UTF-8');  # フォームデータのエンコーディングを UTF-8 に設定

# 名前検索パラメータ取得
my $ch_name = decode('UTF-8', $query->param('ch_name') || '');
my $cv_name = decode('UTF-8', $query->param('cv_name') || '');
my $ch_blood_type = decode('UTF-8', $query->param('ch_blood_type') || '%');
my $type = decode('UTF-8', $query->param('type') || '%');
...
print "<h5>アイドルの名前</h5>";
print '<INPUT type="text" name="ch_name" value="', escape_html($ch_name), '">';
print "</div>";

print '<div style="margin: 10px">';
print "<h5>声優の名前</h5>";
print '<INPUT type="text" name="cv_name" value="', escape_html($cv_name), '">';
print "</div>";
...
sub print_option {
    my ($data, $selected) = @_;
    # デバッグ情報
    print "<!-- Data: ", escape_html($data), ", Selected: ", escape_html($selected), " -->";
    my $selected_attr = ($data eq $selected) ? ' selected' : '';
    print '<option value="', escape_html($data), '"', $selected_attr, '>', escape_html($data), "</option>";
}
...
sub print_results {
    ...
    if ($sth->err) {
        print "<p>ステートメントの準備中にエラーが発生しました: ", $sth->errstr, "</p>";
        return;
    }

    if ($ch_name eq '' && $cv_name eq '' && $ch_blood_type eq '%' && $type eq '%') {
        ...
        if ($sth->err) {
            print "<p>ステートメントの実行中にエラーが発生しました: ", $sth->errstr, "</p>";
            return;
        }
    }
